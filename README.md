...
Начиная с этой версии обновление происходит **прямо в приложении**, без ручного скачивания
и переустановки. При нажатии "Скачать обновление" (в баннере или в настройках) приложение
само скачивает новую версию с прогресс-баром на весь экран, а по готовности предлагает
"Перезапустить сейчас" — после этого старая версия автоматически заменяется новой.

Это работает через `electron-updater`, который читает специальный файл `latest.yml`
из последнего релиза на GitHub — поэтому при каждой публикации **важно прикладывать
не только .exe, но и этот файл** (иначе обновление не найдётся).

Чтобы опубликовать новую версию:
1. Подними `version` в `package.json`
2. Собери: `npm run electron:build`
3. В папке `release/` теперь появится несколько файлов: сам `.exe`, `latest.yml`
   и файл с расширением `.exe.blockmap`
4. GitHub Desktop → Commit → Push (как обычно)
5. GitHub → Releases → новый релиз с тегом (например `v0.1.2`) →
   **прикрепи все три файла** из папки `release/` (.exe, latest.yml, .exe.blockmap) → Publish
6. В Supabase выполни `insert` в `app_versions` с новой версией, прямой ссылкой
   на приложенный `.exe` и описанием изменений (это отдельно — для баннера/журнала обновлений
   в интерфейсе; самим механизмом скачивания это не пользуется, но нужно для текста и уведомлений)

Если что-то пойдёт не так при скачивании (нет интернета, файл не найден и т.п.) — покажется
понятный экран с ошибкой, а не зависание.
